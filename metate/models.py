...
class Flange(models.Model):
    ...
    def __str__(self):
        return f"{self.stone.name} - {self.stone.id} - Flange {self.number} ({self.current_status})"
...
class StoneHandling(models.Model):
    ...
    print(os.path.join(DIR, 'stone_handling_choices.yaml'))
    ACTION_CHOICES = load_yaml(os.path.join(DIR, 'stone_handling_choices.yaml'))
