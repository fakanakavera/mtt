...
class StoneHandlingStep3Form(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if selected_action in ['descartar', 'hanten', 'remover']:
            self.fields.pop('design_number')

        if selected_action == 'hanten':
            self.fields['new_design_number'] = forms.CharField(label='New Design Number', required=True)
